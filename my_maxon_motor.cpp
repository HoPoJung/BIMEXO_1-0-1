...
bool my_maxon_motor::openDevice(QString *str_ErrorCode)
{
    ...
    closeALLDevice();
    char epos2[6] = "EPOS2";
    char maxon[16] = "MAXON SERIAL V2";
    char usb[4] = "USB";
    char usb0[5] = "USB0";
    keyHandle = VCS_OpenDevice(epos2,maxon,usb,usb0,&ErrorCode);
    if( keyHandle == 0 )
    {
...
bool my_maxon_motor::setPositionProfile(double velocity, double acceleration, double decceleration, QString *str_ErrorCode)
{
    DWORD ErrorCode;

    if(!VCS_SetPositionProfile(this->keyHandle, this->node_Id, velocity, acceleration, decceleration, &ErrorCode)){
        *str_ErrorCode = QString("Set position profile failed!, error code=0x%1").arg(ErrorCode, 0, 16);
        return false;
    }
    else
        return true;
...
bool my_maxon_motor::getPositionProfile(double *velocity, double *acceleration, double *decceleration, QString *str_ErrorCode)
{
    DWORD ErrorCode;
    DWORD d_velocity = *velocity;
    DWORD d_accelration = *acceleration;
    DWORD d_decceleration = *decceleration;

    if(VCS_GetPositionProfile(this->keyHandle, this->node_Id, &d_velocity, &d_accelration, &d_decceleration, &ErrorCode)){
        *velocity = d_velocity;
        *acceleration = d_accelration;
        *decceleration = d_decceleration;
        *str_ErrorCode = QString("Get position profile failed!, error code=0x%1").arg(ErrorCode, 0, 16);
        return false;
    }
    else
        return true;
...
bool my_maxon_motor::init_profileVelocityMode(int nodeId, double MaxProfileVelocity, double MaxFollowingError, double MaxAcceleration, double MaxDecceleration)
{
    ...
    if(keyHandle)
    {
        VCS_SetOperationMode(this->keyHandle, this->node_Id, OMD_PROFILE_VELOCITY_MODE, &ErrorCode);
        VCS_SetMaxProfileVelocity(this->keyHandle, this->node_Id, MaxProfileVelocity, &ErrorCode);
        VCS_SetMaxFollowingError(this->keyHandle, this->node_Id, MaxFollowingError, &ErrorCode);
        VCS_SetMaxAcceleration(this->keyHandle, this->node_Id, MaxAcceleration, &ErrorCode);
        VCS_SetVelocityProfile(this->keyHandle, this->node_Id, MaxAcceleration, MaxDecceleration, &ErrorCode);
        VCS_ActivateProfileVelocityMode(this->keyHandle, this->node_Id, &ErrorCode);
        return OK;
    }
}
...
void my_maxon_motor::MoveToPositionWithVelocity(double TargetPosition, double velocity, double acceleration, double decceleration, bool Absolute, QString *str_ErrorCode)
{
    DWORD ErrorCode = 0;
...
//                ui.lineEditPositionStart->setText(QString("%1").arg(PositionIs));
        ...
        if( !VCS_MoveToPosition(this->keyHandle, this->node_Id, TargetPosition, Absolute, Immediately, &ErrorCode) && !VCS_SetPositionProfile(this->keyHandle, this->node_Id, velocity, acceleration, decceleration, &ErrorCode))
        {
            *str_ErrorCode = QString("Move to position failed!, error code=0x%1").arg(ErrorCode, 0, 16);
        }
    }
...
void my_maxon_motor::MoveWithVelocity(double TargetVelocity, QString *str_ErrorCode)
{
    DWORD ErrorCode = 0;
    if(!VCS_MoveWithVelocity(this->keyHandle, this->node_Id, TargetVelocity, &ErrorCode)){
        *str_ErrorCode = QString("Move with velocity failed, error code=0x%1").arg(ErrorCode, 0, 16);
    }
}
